...
def dalle_multi_image_generation(prompt):
    text = [item for item in prompt if item]
    generated_image=[]
    for i in range(len(text)):
        ...
        generated_image.append(image_url)
    return generated_image

def convert_image_url_to_base64(image_url: str) -> str:
...
# def multiple_image_generation(prompt):
#     try:
#         generated_image=[]
#         i=0
#         while i<3:
...
def multiple_image_generation(prompt):
    ...
    try:
        # Using a named variable 'i' to iterate over the range
        for i in range(3):
            response = client.images.generate(
                model="dall-e-3",
                prompt=prompt,
...
def convert_image_url_to_base64(image_url: str) -> str:
    ...
    if response.status_code == 200:
        ...
        return Exception(f"Failed to ocnvert base 64 image:{response.status_code}")
